...
const LeaveServerModal = ({ setIsModalOpen }) => {
   ...
   const handleLeaveServer = async () => {
    try {
        ...
        let response = await axios.get(`http://localhost:3000/servers/${currServer.id}/${currUser.userId}`);
        ...
        <Dialog open onOpenChange={handleClose}>
            <DialogContent className="bg-white text-black p-4 overflow-hidden">
                ...
                <DialogDescription className="text-center text-zinc-500">
                    Are you sure you want to leave <span className="font-semibold text-indigo-500">{currServer?.name}</span>
                </DialogDescription>
